...
EOF
  ...
  conn.exec(cmd, (err, stream) => {
    if (err) throw err;
    const fs = require('fs');
    fs.writeFileSync('absolute_final.log', '');
    stream.on('close', (code) => {
      conn.end();
    })
    .on('data', data => fs.appendFileSync('absolute_final.log', data.toString()))
    .stderr.on('data', data => fs.appendFileSync('absolute_final.log', data.toString()));
  });
