...
conn.on('ready', () => {
  ...
  conn.exec(cmd, (err, stream) => {
    if (err) throw err;
    const fs = require('fs');
    fs.writeFileSync('backend_fix.log', '');
    stream.on('close', (code) => {
      conn.end();
    })
    .on('data', data => fs.appendFileSync('backend_fix.log', data.toString()))
    .stderr.on('data', data => fs.appendFileSync('backend_fix.log', data.toString()));
  });
