...
conn.on('ready', () => {
  ...
  conn.exec(cmd, (err, stream) => {
    if (err) throw err;
    const fs = require('fs');
    fs.writeFileSync('bash-crash.log', '');
    stream.on('close', (code) => {
      conn.end();
    })
    .on('data', data => fs.appendFileSync('bash-crash.log', data.toString()))
    .stderr.on('data', data => fs.appendFileSync('bash-crash.log', data.toString()));
  });
