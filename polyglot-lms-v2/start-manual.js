...
module.exports = {
  ...
  const cmd = `
    ...
    echo "--- REMOTE PACKAGE.JSON ---" &&
    cat package.json &&
    
    echo "--- REWRITING PACKAGE.JSON ---" &&
    sed -i 's/"start".*/"start": "next start",/g' package.json || sed -i '/"scripts": {/a \\    "start": "next start",' package.json &&
...
EOF
  ...
  conn.exec(cmd, (err, stream) => {
    if (err) throw err;
    const fs = require('fs');
    fs.writeFileSync('manual.log', '');
    stream.on('close', (code) => {
      conn.end();
    })
    .on('data', data => fs.appendFileSync('manual.log', data.toString()))
    .stderr.on('data', data => fs.appendFileSync('manual.log', data.toString()));
  });
