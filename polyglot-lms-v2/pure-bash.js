...
EOF
  ...
  conn.exec(cmd, (err, stream) => {
    if (err) throw err;
    const fs = require('fs');
    fs.writeFileSync('pure_bash.log', '');
    stream.on('close', (code) => {
      conn.end();
    })
    .on('data', data => fs.appendFileSync('pure_bash.log', data.toString()))
    .stderr.on('data', data => fs.appendFileSync('pure_bash.log', data.toString()));
  });
